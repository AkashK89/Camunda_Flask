...
from camunda_external.external_task_executor import ExternalTaskExecutor
...
class ExternalTaskWorker:
    ...
    def subscribe(self, topic_names, action, process_variables=None):
        while not self.stopped:
            result = self._fetch_and_execute_safe(topic_names, action, process_variables)
    ...
    def _fetch_and_execute_safe(self, topic_names, action, process_variables=None):
        try:
            resp = self.fetch_and_execute(topic_names, action, process_variables)
    ...
    def fetch_and_execute(self, topic_names, action, process_variables=None):
        ...
        return self._execute_tasks(tasks, action)
    ...
    def _execute_tasks(self, tasks, action):
        resp = True
        for task in tasks:
            resp = self._execute_task(task, action)
            if not resp:
                resp = False
                break
        return resp
    ...
    def _execute_task(self, task, action):
        try:
            return self.executor.execute_task(task, action)
